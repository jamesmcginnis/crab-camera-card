...
class CrabCameraCard extends HTMLElement {
  ...
  static getConfigElement() {
    return document.createElement('crab-camera-card-editor');
  }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .scroll-fade::after {
          ...
          width: 32px;
          background: linear-gradient(to right, transparent, var(--ha-card-background, var(--card-background-color, #1c1c1e)));
          pointer-events: none;
          z-index: 2;
        ...
        .cam-dot.online {
          background: #34C759;
          box-shadow: 0 0 0 1.5px rgba(0,0,0,0.35), 0 0 7px rgba(52,199,89,.7);
        }
        .cam-dot.offline {
          background: #48484a;
  ...
  _tileHtml(id) {
    ...
    const name     = state?.attributes?.friendly_name || id.split('.').slice(1).join('.').replace(/_/g,' ');
    ...
    const inner = (!state || !online)
      ? `<div class="cam-unavail">
           <svg viewBox="0 0 24 24" width="26" height="26" fill="currentColor" style="opacity:.3">
             <path d="M18.44 2.12L2.12 18.44 3.56 19.88 5 18.44V18A1 1 0 0 0 6 19H18A1 1 0 0 0 19 18V7A1 1 0 0 0 18.44 6.12L21 3.56 18.44 2.12M17 10.5V7H8.56L17 15.44V10.5M21 6.5L17 10.5V13.59L21 17.5V6.5Z"/>
           </svg>
           <span>Unavailable</span>
         </div>`
      : `<img class="cam-img" id="${this._imgId(id)}"
              src="${isLive ? this._streamUrl(id) : this._proxyUrl(id)}"
    ...
    return `
      <div class="cam-tile" data-entity="${id}">
        <div class="cam-wrap">
          ...
          ${showDot ? `<div class="cam-dot ${online ? 'online' : 'offline'}"></div>` : ''}
        </div>
  ...
  _bindTiles() {
    ...
    // Cancel long-press when user is swiping the scroll container
    const scroll = this.shadowRoot.getElementById('crabScroll');
    if (scroll) {
      scroll.addEventListener('scroll', () => {
        this.shadowRoot.querySelectorAll('.cam-tile').forEach(t => t.dispatchEvent(new Event('pointercancel')));
      }, { passive: true });
    }
  }
  ...
  _startTimers() {
    ...
    (this._config?.entities || []).forEach(id => {
      this._refreshTimers[id] = setInterval(() => {
        ...
        if (img) { img.style.opacity = '1'; img.src = this._proxyUrl(id); }
      }, ms);
    });
  }
  _stopTimers() {
  ...
  _updateDots() {
    ...
    (this._config?.entities || []).forEach(id => {
      ...
      dot.className = `cam-dot ${online ? 'online' : 'offline'}`;
    });
  }
  ...
  _openPopup(id) {
    ...
    const state  = this._hass?.states[id];
    const name   = state?.attributes?.friendly_name || id.split('.').slice(1).join('.').replace(/_/g,' ');
    const stream = this._streamUrl(id);
    this._popupMuted = false;
    ...
    el.innerHTML = `
      <style>
        #crab-backdrop {
          ...
          animation: crabBDIn .22s ease forwards;
        }
        @keyframes crabBDIn { from { opacity:0 } to { opacity:1 } }
        ...
        .crab-card {
          ...
          overflow: hidden;
          display: flex; flex-direction: column;
          box-shadow: 0 28px 90px rgba(0,0,0,.75), 0 0 0 .5px rgba(255,255,255,.07);
          animation: crabCardIn .28s cubic-bezier(.34,1.4,.64,1) forwards;
        }
        @keyframes crabCardIn {
        ...
        .crab-ph {
          ...
          padding: 14px 16px 11px; gap: 12px;
        }
        .crab-pt {
        ...
        .crab-close:hover  { background: rgba(255,255,255,.2); }
        .crab-close:active { transform: scale(.88); }
        ...
        .crab-vw::before { content:''; display:block; padding-top:56.25%; }
        .crab-vi {
          position: absolute; inset: 0;
          width: 100%; height: 100%;
        ...
        .crab-actions {
          ...
          padding: 16px 20px 20px; gap: 8px;
        }
        .crab-ab {
        ...
        .crab-ab:hover .crab-ai { background: rgba(255,255,255,.17); }
        .crab-ab:active .crab-ai { transform: scale(.88); background: rgba(255,255,255,.07); }
        .crab-ab.muted .crab-ai { background: rgba(255,59,48,.22); color: #ff3b30; }
        .crab-al {
      ...
      <div id="crab-backdrop">
        <div class="crab-card">

          <!-- header -->
          <div class="crab-ph">
            <div class="crab-pt">${name}</div>
            <button class="crab-close" id="crabX" aria-label="Close">
          ...
          <!-- video -->
          <div class="crab-vw" id="crabVW">
            <div class="crab-vspinner" id="crabSpin">
              <div class="crab-spinner-ring"></div>
              <span>Connecting…</span>
            </div>
            <img class="crab-vi" id="crabVI" alt="${name}"
                 style="opacity:0;transition:opacity .3s ease">
          </div>

          <!-- actions -->
          <div class="crab-actions">
            <div class="crab-ab" id="crabMuteBtn">
              <div class="crab-ai">
                <svg id="crabMuteIco" viewBox="0 0 24 24" width="22" height="22" fill="currentColor">
                  <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM18.5 12c0 2.77-1.5 5.19-3.78 6.54l1.42 1.42C19.04 18.12 21 15.26 21 12c0-3.26-1.96-6.12-4.86-7.96l-1.42 1.42C16.97 6.76 18.5 9.2 18.5 12z"/>
                </svg>
              </div>
              <span class="crab-al">Sound</span>
            </div>

            <div class="crab-ab" id="crabFullBtn">
              <div class="crab-ai">
                <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor">
                  <path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/>
                </svg>
              </div>
              <span class="crab-al">Full Size</span>
            </div>
          </div>
    ...
    // Show image once loaded (hide spinner)
    const img  = el.querySelector('#crabVI');
    const spin = el.querySelector('#crabSpin');
    img.onload = () => {
      img.style.opacity = '1';
      spin.style.display = 'none';
    };
    img.onerror = () => {
      spin.innerHTML = `<svg viewBox="0 0 24 24" width="36" height="36" fill="currentColor" style="opacity:.3"><path d="M21 6.5l-4-4-9.5 9.5-2.5 4.5 4.5-2.5L21 6.5z"/></svg><span style="margin-top:4px">Stream unavailable</span>`;
    };
    img.src = stream;

    // Close
    el.querySelector('#crabX').addEventListener('click', () => this._destroyPopup());
    el.querySelector('#crab-backdrop').addEventListener('click', e => {
      if (e.target.id === 'crab-backdrop') this._destroyPopup();
    });
    ...
    el.querySelector('#crabMuteBtn').addEventListener('click', () => {
      ...
      const lbl = btn.querySelector('.crab-al');
      btn.classList.toggle('muted', this._popupMuted);
      if (this._popupMuted) {
    ...
    el.querySelector('#crabFullBtn').addEventListener('click', () => {
      const wrap = el.querySelector('#crabVW');
    ...
    this.shadowRoot.innerHTML = `
      <style>
        * { box-sizing: border-box; }
        ...
        .segmented {
          display: flex; background: rgba(118,118,128,.2);
        ...
        .dragging { opacity: .45; background: rgba(255,255,255,.05) !important; }
        .drag-handle {
          cursor: grab; color: rgba(255,255,255,.3);
          display: flex; align-items: center; flex-shrink: 0;
          padding: 4px 2px;
        }
        .cam-icon { flex-shrink: 0; color: rgba(255,255,255,.35); }
        ...
        /* Smaller toggle for list items */
        .check-item .toggle-switch { width: 44px; height: 26px; }
      ...
      <div class="container">
        ...
        <div>
          ...
          <div class="card-block">
            <div class="input-row">
              ...
              <input type="text" id="crabTitle" value="${this._config.title || 'Cameras'}" placeholder="Cameras">
            </div>
        ...
        <div>
          ...
          <div class="card-block">
            ...
            <div class="toggle-item" id="refreshRow" style="display:flex">
              <div class="lhs">
              ...
              <div class="num-inline">
                <input type="number" id="crabRefresh" min="2" max="3600" step="1"
                       value="${this._config.refresh_interval || 10}">
                <span>sec</span>
              </div>
            </div>
  ...
  _setupListeners() {
    ...
    if (list) {
      list.addEventListener('change', e => {
        ...
        if (cb.checked) {
          const allItems = [...list.querySelectorAll('.check-item')];
          const last = allItems.filter(i => i.querySelector('input[type="checkbox"]')?.checked && i !== item).pop();
